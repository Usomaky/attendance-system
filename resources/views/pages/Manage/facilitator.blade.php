<x-app-layout>
    ...
    <div class="py-2 lg:py-12">
        <div class="w-full mx-auto sm:px-6 lg:px-8">
            <div class="">
                <h1 class="text-4xl font-bold md:text-4xl">Facilitator</h1>

                <div>
                    <x-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
                        {{ __('Dashboard') }}
                    </x-nav-link>/
                    <x-link :href="route('manageFacilitator')" :active="request()->routeIs('manageFacilitator')">
                        {{ __('Facilitator') }}
                    </x-nav-link>
                </div>
                ...
                <div class="flex justify-end my-6">
                    <x-add-button x-data="" class=""
                        x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion')">{{ __('Add New Facilitator') }}</x-add-button>

                    <x-modal name="confirm-user-deletion" :show="$errors->isNotEmpty()" focusable>
                        <form method="post" action="{{ route('facilitator.store') }}" class="p-6"
                            ...
                            <p class="mt-1 text-sm text-gray-600">
                                {{ __('Create an account for the facilitator.') }}
                            </p>
                            {{-- Full Name --}}
                            <div class="mt-6">
                ...
                @if (count($facilitators) < 1)
                    <p class="text-lg text-center text-gray-800">No facilitator found.</p>
                @else
                            ...
                            <tbody>
                                @foreach ($facilitators as $facilitator)
                                    <tr class="capitalize">
                                        ...
                                        <td class="lowercase">{{ $facilitator->email }}</td>
                                        <td>{{ $facilitator->role }}</td>
                                        <td>{{ $facilitator->course->name }}</td>
                                        <td>
                                            <!-- Example actions (edit, delete, etc.) -->
                                            {{-- <form action="{{ route('facilitator.destroy', $facilitator) }}"
                                            ...
                                            <div>
                                                ...
                                                <x-modal name="confirm-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
                                                    <form method="post"
                                                        action="{{ route('facilitator.destroy', $facilitator) }}" class="p-6">
                                                        @csrf
                                                        @method('delete')

                                                        <h2 class="text-lg font-medium text-gray-900">
                                                            {{ __('Are you sure you want to delete this facilitator?') }}
                                                        </h2>

                                                        <p class="mt-1 text-sm text-gray-600">
                                                            {{ __('Once this facilitator is deleted, all of their resources and data will be permanently deleted.') }}
                                                        </p>
